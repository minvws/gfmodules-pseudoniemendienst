...
Exchange a personal ID for a reversible pseudonym bound to a recipient organization/scope. Requires the `prs:reversible-pseudonym` OAuth scope. The response is a JWE encrypted to the recipient's registered public key for that scope (content type `application/jwe`, status `201`); its decrypted `subject` claim is `pseudonym:reversible:<...>`. The pseudonym is deterministic for the same personal ID, organization and scope, and can only be reversed to the personal ID by the PRS.
...
- the calling organization (the verified `x-gf-sub` identity) must have `may_provide_personal_id` set;
- the recipient organization must have a `max_rid_usage` of at least `rp`, since the pseudonym can be reversed to the personal ID by the PRS.

Neither flag can be set by the organizations themselves. Responses: `403` when the scope is missing or either authorization fails (the sender is checked first, so an unauthorized caller cannot probe which organizations exist), `400` when `personalId` is malformed, `404` when the recipient organization is unknown or has no public key for the scope.
